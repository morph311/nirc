  ...
  io.sockets.on('connection', function (client) {
    ...
    client.on('connectToIRC', function (data) {			
      ...
      var opts = {
        ...
        channels:     data.options.channels.replace("#r.opiates","#r.youholding").split(","),
        password:     data.options.password || null,
        userName:     data.options.userName || null,
        secure:       data.options.ssl ? { rejectUnauthorized: false } : true,
        selfSigned:   data.options.ssl ? true : null,
        certExpired:  data.options.ssl ? true : null
        //channels:     data.options.channels.replace("#r.opiates")
      };
